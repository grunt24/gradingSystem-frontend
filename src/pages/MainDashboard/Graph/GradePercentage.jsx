...
const PERCENTAGE_API =
  "https://localhost:7255/api/GradeCalculation/grade-percentage";
const EQUIVALENTS_API =
  "https://localhost:7255/api/GradeCalculation/equivalents";
...
const GradePercentage = () => {
  ...
  useEffect(() => {
    const fetchPercentageData = async () => {
      try {
        const res = await fetch(PERCENTAGE_API);
        ...
        if (json && json.data) {
          const chartData = [
            ...
            { name: "Midterm Exam", value: json.data.midtermWeighted },
          ];
    ...
    const fetchEquivalentData = async () => {
      try {
        const res = await fetch(EQUIVALENTS_API);
        const json = await res.json();
